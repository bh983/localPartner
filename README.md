...
> 这并不是唯一的解决方案。b站有一起看功能，资源丰富，服务稳定。如果想做到更可控的本地视频播放，同时实现复杂的画质改善、外挂字幕、一起看等功能，考虑使用mpv+滤镜+Syncplay。弹弹play同样是成熟且友好的弹幕本地播放器。另外受kazumi所限，本工具暂时没办法播放外挂字幕视频，若确有需求，考虑上述其他工具或者把外挂字幕硬嵌入视频。
## 功能简要介绍
1.搜索用户指定的目录，检索出需要播放的番剧视频，通过解析文件名获取基本信息。

2.连接萌娘百科/Bangumi API，检索出番剧的通用名。（可选）

3.根据kazumi规则文档指示，部署http服务，充当临时的“API”，供Kazumi当作视频源连接。
...
2.指定一个空闲的端口，点击启动服务。端口一般默认即可；记得允许程序访问网络
...
4.在kazumi搜索要看的番剧，找到第3步新建的源，点击播放即可。软件会根据读取的视频文件确定集数和集号并对应提供给kazumi，不受在线识别影响。若没有搜索到，则手动输入localPartner中识别的番剧名并检索即可。

## FAQ
Q：番剧名识别/在线检索出错？

A：只要同一部作品被正确归到一起（一个简单的达成方法是确保在同一子文件夹下有且仅有一部作品）且集数识别正常就**不影响观看**，在kazumi里手动检索它被错误识别的名字即可。另外，对于不确定是否规范或者总是无法准确归类的番剧文件，让ai写一个批量脚本重命名成类似下列格式：
